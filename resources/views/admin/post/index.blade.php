...
@section('content')
        <div class="scrollable-page main-scrollable-page">
            <div id="page-content" class="page-wrapper clearfix">
                <div class="clearfix">
                    <ul id="client-tabs" data-bs-toggle="ajax-tab" class="nav nav-tabs bg-white title" role="tablist">
                        ...
                        <li>
                            <a role="presentation" href="{{ route('post_list') }}" data-bs-target="#post_list">{{ __('Post') }}</a>
                        </li>

                            
                        <div class="tab-title clearfix no-border">
                            <div class="title-button-group"> 
                                <a href="#" class="btn btn-default" title="{{ __('Create Post') }}" data-act="ajax-modal" data-title="{{ __('Create Post') }}" data-method="get" data-action-url="{{ route('post.create') }}"><i data-feather='plus-circle' class='icon-16'></i> {{ __('Post Create') }}</a>
                            </div>
                        </div>
                    </ul>
                    <div class="tab-content">
                        ...
                        <div role="tabpanel" class="tab-pane fade" id="post_list"></div>
                    </div>
                </div>
            </div>
            <script type="text/javascript">
            $(document).ready(function() {
                setTimeout(function() {
                    var tab = "";
                    if(tab === "post_list" || tab === "post_list-has_open_projects") {
                        $("[data-bs-target='#post_list']").trigger("click");
